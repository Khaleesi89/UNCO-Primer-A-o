...
function opcionesInicio($seleccion){
    switch($seleccion){
...
function noHayViajes($opcion){
    $opcion = verificadorInt($opcion);
    while(($opcion < 0) || ($opcion > 3)){
        echo "No hay viajes! porfavor ingrese las opciones 0, 1, 2 o 3: ";
...
do{
    ...
    $seleccion = "";
    if($dimensionViaje == 0){
        echo "No hay viajes! solamente podra ingresar la opcion 0, 1, 2 o 3: ";
        $seleccion = menuInicio();
        $seleccion = noHayViajes($seleccion);
        opcionesInicio($seleccion);
    }else{
        $seleccion = menuInicio();
        opcionesInicio($seleccion);
    }
